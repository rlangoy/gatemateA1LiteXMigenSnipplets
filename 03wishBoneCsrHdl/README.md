...
1.Wrap a **HDL entity** (`hdl/crc.v` or `hdl/crc.vhdl`  ) as a Migen black-box using `Instance`
...
The CRC32 step logic is generated by the [Bues Online CRC HDL Generator](https://bues.ch/cms/hacking/crcgen.html). Both VHDL (`hdl/crc.vhdl`) and Verilog (`hdl/crc.v`) variants are provided; the simplest implemnetation uses the Verilog version (wishBoneCrsCrc32Verilog.py)
...
| File | Description |
|---|---|
| `wishBoneCrsCrc32Verilog.py` | FPGA design: UART bridge + CSR-mapped CRC32 peripheral backed by Verilog black-box |
...
## One-Time Setup: (for the wishBoneCrsCrc32Vhdl.py) GHDL Yosys Plugin
...
```bash
python wishBoneCrsCrc32Verilog.py or
