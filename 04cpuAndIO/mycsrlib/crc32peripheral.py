...
class CRC32Peripheral(Module, AutoCSR):
    ...
    Two registers within location slot 2:
      crc32_data       @ 'CSR-AutoAddress' (32-bit rw)
      ...
      crc32_reset_ctrl @ 'CSR-AutoAddress' +4 (32-bit w)
    ...
    def __init__(self, platform):
        # data : CRS Address CRS_MAP + Offset (0)  (first registered CRS)
        # data CSR: write[7:0] = data byte in, read[31:0] = checksum out
        # if the bus writes data to the 'CRS Address'  self.data is automaticaly updated
        self.data = CSR(32, name="data")
        # reset_ctrl CSR: writing any value resets the CRC accumulator to 0xFFFFFFFF
        # if the bus writes data to the 'CRS Address' self.reset_ctrl is automaticaly updated
        ...
        # Byte-swap helper: reverse endianness of a 32-bit signal
        def bswap32(s):
            return Cat(s[24:32], s[16:24], s[8:16], s[0:8])

        self.sync += [
            If(self.reset_ctrl.re,            # Writing to 'CRS Address' triggers "register enable" (write strobe) for reset_ctrl.re
                crc_in.eq(0xFFFFFFFF),
                out_buf.eq(0xFFFFFFFF),
            ).Elif(self.data.re,	      # Writing to 'CRS Address' triggers "register enable" (write strobe) for data.re
                crc_in.eq(crc_out),
                out_buf.eq(bswap32(crc_out)),
            )
        ]
